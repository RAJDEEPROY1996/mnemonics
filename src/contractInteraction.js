...
const OwnershipTransferred = async(newOwner) => {  
  accounts = await web3.eth.getAccounts()
  await instance.methods.transferOwnership(newOwner).send({from: accounts[0],gas: 8500000}).then(()=>{
    console.log("---------------------------");
    console.log("Ownership have been transferred from ",accounts[0],"to ",newOwner);
    console.log("---------------------------");
  });  
}
...
const mintToken = async(amount) => {  
    accounts = await web3.eth.getAccounts()
    await instance.methods.mint(accounts[0],amount).send({from: accounts[0],gas: 8500000}).then(()=>{
      console.log("---------------------------");
      console.log("minted token for ",accounts[0]);
      console.log("---------------------------");
    });  
}
...
Connect();
//OwnershipTransferred('0x8F06f3E3c0ddDCaF8525f1AE3723C9F2f19D610b');
